...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def get_grouped_lines(self):
        ...
        for cuenta in cuentas:
            # Obtén el valor de debit_positive para la cuenta
            debit_positive = cuenta.debit_positive

            # Agrupa las cuentas por el valor de debit_positive en date_summary
            if debit_positive not in date_summary:
                date_summary[debit_positive] = []
            date_summary[debit_positive].append(cuenta.name)  # Puedes guardar los datos que desees aquí

        # Ahora date_summary contiene las cuentas agrupadas por debit_positive
        result.append(date_summary)
    ...
    def get_account_summary(self):
        ...
        for line in move_lines:
            date = line.date  
            if date in summary:
                summary[date]['debit'] += line.debit
                summary[date]['credit'] += line.credit
            else:
                summary[date] = {
                    'debit': line.debit,
                    'credit': line.credit
    ...
    def _get_move_line_data(self, move_line):
        move_line_data = {
            "id": move_line["id"],
            "date": move_line["date"],
            "total_balance":move_line['total_balance'],
    ...
    def _calculate_centralization(self, centralized_ml, move_line, date_to):
        jnl_id = move_line["account_id"]
        date = move_line["date"]
       # debit_positive = move_line["debit_positive"]
        total_balance  =move_line["total_balance"]
        ...
        account = self.env["account.account"].browse(jnl_id)
        debit_positive = account.debit_positive 
        # Construye una clave única usando la fecha en lugar del mes
        date_key = date.strftime('%Y-%m-%d')
    
        if jnl_id not in centralized_ml.keys():
            centralized_ml[jnl_id] = {}
        if date_key not in centralized_ml[jnl_id].keys():
            centralized_ml[jnl_id][date_key] = {
            "account_id": jnl_id,
            "ref_label": "Centralized Days",
            "date": date,
            "debit": 0.0,
          ...
          # Verifica si el código de la cuenta comienza con 2, 3 o 5 y multiplica "debit" por -1 si es así
        if debit_positive == True:         
            centralized_ml[jnl_id][date_key]["debit"] += move_line["debit"]
            centralized_ml[jnl_id][date_key]["credit"] += move_line["credit"]
            #centralized_ml[jnl_id][date_key]["balance"] = centralized_ml[jnl_id][date_key]["balance"] + move_line["debit"] - move_line["credit"]
            centralized_ml[jnl_id][date_key]["balance"] += move_line["debit"] - move_line["credit"]             
            centralized_ml[jnl_id][date_key]["total_balance"] = abs(centralized_ml[jnl_id][date_key]["balance"])
           

            centralized_ml[jnl_id][date_key]["bal_curr"] += move_line["bal_curr"]
    ...
    def _get_report_values(self, docids, data):
       ...
       # general_ledger = sorted(general_ledger, key=lambda k: (k["date"], k["code"]))
