...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _compute_account_amount(
        ...
        for tb in tb_period_acc:
            ...
            total_amount[acc_id]["ending_balance"] =  tb["balance"] # si se convierte a abs() se suma
            if foreign_currency:
        ...
        for tb in tb_initial_acc:
            ...
            if acc_id not in total_amount.keys():
                ...
                total_amount[acc_id]["initial_balance"] = tb["balance"] # n
                total_amount[acc_id]["ending_balance"] = tb["balance"]#n
                if foreign_currency:
    ...
    def _compute_partner_amount(
        ...
        for tb in tb_period_prt:
            ...
            if tb["partner_id"]:
                ...
                total_amount[acc_id][prt_id]["ending_balance"] = abs(tb["balance"])
                if foreign_currency:
    ...
    def _get_groups_data(self, accounts_data, total_amount, foreign_currency):
        ...
        for group_id in account_group_relation.keys():
            for account_id in account_group_relation[group_id]:
                ...
                groups_data[group_id]["balance"] += abs(total_amount[account_id]["balance"])
                groups_data[group_id]["ending_balance"] += abs(total_amount[account_id][
                    "ending_balance"
                ])
                if foreign_currency:
                    groups_data[group_id]["initial_currency_balance"] += abs(total_amount[
                        account_id
                    ]["initial_currency_balance"])
                    groups_data[group_id]["ending_currency_balance"] += abs(total_amount[
                        account_id
                    ]["ending_currency_balance"])
        ...
        for group in groups:
            ...
            for account in accounts_data.values():
                if group.code_prefix_start == account["code"][:len_group_code]:
                    ...
                    groups_data[group_id]["balance"] += abs(total_amount[acc_id]["balance"])
                    groups_data[group_id]["ending_balance"] += abs(total_amount[acc_id][
                        "ending_balance"
                    ])
                    if foreign_currency:
                        groups_data[group_id]["initial_currency_balance"] += abs(total_amount[acc_id]["initial_currency_balance"])
                        groups_data[group_id]["ending_currency_balance"] +=  abs(total_amount[acc_id]["ending_currency_balance"])
        return groups_data
                ...
                accounts_data[account_id].update(
                    {
                        ...
                        "balance": abs(total_amount[account_id]["balance"]),
                        "ending_balance": abs(total_amount[account_id]["ending_balance"]),
                        "type": "account_type",
                    }
                )
                if foreign_currency:
